...
#' @examples Datasets need to be arranged so that there is a one-once organizational correspondence between the normals and tumors/ before and after treatment patients
...
pairedStat <- function(normalMat, tumorMat){
  ...
  temp.means = temp.sd = MatsdEst = matrix(0,n,s)
  matNu = matrix(0,n,s)

  for (i in 1:s){
    for (j in 1:n){
      temp.means[j,i] <- mean(c(normalMat[j,i],tumorMat[j,i]))
      temp.sd[j,i] <- sd(c(normalMat[j,i],tumorMat[j,i]))
    }
    l.mod <- loess(temp.sd[ ,i] ~ temp.means[ ,i])
    MatsdEst[ ,i] <- predict(l.mod)

    for (k in 1:n){
      matNu[k,i] <- abs(normalMat[k,i] - tumorMat[k,i])/MatsdEst[k,i]
    }
  }
